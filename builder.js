...
function card(r, idx) {
  ...
  const actions = `<div class="actions">${concl === "success" ? `${downloads.apk ? `<a class="dl-btn" href="${downloads.apk}" download>APK</a>` : ""}${downloads.aab ? `<a class="dl-btn" href="${downloads.aab}" download>AAB</a>` : ""}` : `<a href="${esc(url)}" target="_blank" class="log-btn">Логи →</a>`}</div>`;
